    ...
    for i in range(5):
        for j in range(5):
            if matrix[i][j] == letter:
                return (i, j)
    # If the letter is not found, you can handle it gracefully, raise an error, or return a special value.
    return None
...
def playfair(key, message, encrypt=True):
    ...
    # Apply 'X' insertion for encryption
    if encrypt:
        message = separate_same_letters(message)
    
    cipher_text = ''
    for (l1, l2) in zip(message[0::2], message[1::2]):
        index1 = indexOf(l1, matrix)
        index2 = indexOf(l2, matrix)
        if index1 is None or index2 is None:
            # Handle the case where a letter is not found in the matrix (e.g., skip it)
            continue
        row1, col1 = index1
        row2, col2 = index2
        ...
        else:  # Rule 4, the letters are in different rows and columns
            cipher_text += matrix[row1][col2] + matrix[row2][col1]
    
    # Remove 'X' characters during decryption
    if not encrypt:
